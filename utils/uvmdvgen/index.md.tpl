...
% if has_alerts:
${'###'} Alert_agents
${name.upper()} testbench instantiates [alert_agents]({{< relref "hw/dv/sv/alert_esc_agent/doc" >}}):
[list alert names].
...
All test sequences are extended from `${name}_base_vseq`, which inlines `dv_base_vseq` and serves as a starting point.
...
```console
$ dvsim $REPO_TOP/hw/ip/${name}/dv/${name}_sim_cfg.hjson -i ${name}_smoke
```
